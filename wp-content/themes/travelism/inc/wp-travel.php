...
function travelism_search_form() { 
...
?>
	...
	<div class="wp-travel-search">
		<form method="get" name="wp-travel_search" action="<?php echo esc_url( home_url( '/' ) );  ?>" > 
			...
			<p class="wp-travel-search"><input type="submit" name="wp-travel_search" id="wp-travel-search" class="button button-primary" value="<?php esc_attr_e( 'Search', 'travelism' ) ?>"  /></p>
		</form>
	</div>	
	<?php
}
add_filter( 'wp_travel_search_form', 'travelism_search_form', 999 );
 ...
 */
function travelism_trip_duration( $post_id ) {
	if ( ! $post_id ) {
		return;
	}
	...
	if ( 'yes' === $fixed_departure ) : 
		...
		if ( $trip_duration || $trip_duration_night ) :
			printf( __( '%1$s Day(s) %2$s Night(s)', 'travelism' ), $trip_duration, $trip_duration_night );
		endif;
...
add_action( 'wp_travel_after_main_content', 'travelism_archive_wrapper_close' );
/**
 * Add html for Keywords.
 */
function travelism_archive_wrapper_close() { ?>
	</div>
<?php }
...
add_action( 'travelism_trip_after_title', 'wptravel_trip_price', 10 );
add_action( 'travelism_trip_after_title', 'wptravel_single_excerpt', 20 );
add_action( 'travelism_trip_after_title', 'wptravel_trip_map', 30 );
