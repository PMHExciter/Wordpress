 ...
 * @since      1.0.0
 */
class blogshare_Random_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-blogshare-random widget-posts-thumbnail',
			'description' => __( 'Display random posts with thumbnails.', 'blogshare' )
		);

		// Create the widget.
		parent::__construct(
			'blogshare-random',                                   // $this->id_base
			__( '&raquo; Random Posts', 'blogshare' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $random = get_transient( 'blogshare_random_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'blogshare_random_widget_' . $this->id, $random );
	...
	function flush_widget_transient() {
		delete_transient( 'blogshare_random_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Random Posts', 'blogshare' ),
			'limit'     => 5,
			'show_date' => true
		);
