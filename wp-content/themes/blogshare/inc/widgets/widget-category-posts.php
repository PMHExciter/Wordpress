 ...
 * @since      1.0.0
 */
class BlogShare_Category_Posts_widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-blogshare-category-posts widget-posts-thumbnail',
			'description' => __( 'Display posts in selected category', 'blogshare' )
		);

		// Create the widget.
		parent::__construct(
			'blogshare-category-posts',          // $this->id_base
			__( '&raquo; Category Posts', 'blogshare' ), // $this->name
 ...
 ?>
			...
			<?php
				if ( ( ! empty( $instance['title'] ) ) && ( $cat_id != 0 ) ) {
					...
					echo ( '<h3 class="widget-title"><span>' . esc_html__( 'Recent Posts', 'blogshare' ) . '</span></h3>' );
				} else {
			...
			<?php
				...
				// Store the transient.
				set_transient( 'blogshare_category_posts_widget_' . $this->id, $wp_query );
	...
	?>
		...
		<p>
			<label for="<?php echo esc_html( $this->get_field_id( 'cat' ) ); ?>"><?php esc_html_e( 'Select a category', 'blogshare' ); ?></label>
			<select class="widefat" id="<?php echo esc_html( $this->get_field_id( 'cat' ) ); ?>" name="<?php echo esc_html( $this->get_field_name( 'cat' ) ); ?>" style="width:100%;">
				...
				<option value="0"><?php esc_html_e( 'All categories &hellip;', 'blogshare' ); ?></option>
