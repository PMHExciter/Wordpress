  ...
  function ocdi_register_plugins( $plugins ) {
    ...
    if (file_exists(get_stylesheet_directory() .'/addons/wdk-payments.zip')) {
        $theme_plugins[] = [
              ...
              'source'   =>  get_stylesheet_directory() . '/addons/wdk-payments.zip',
              'required' => false,
              'preselected' => true,
        ];
        $theme_plugins[] = [
...
function ocdi_after_import_setup($selected_import) {
    ...
    $home_alt_page = get_page_by_title( 'Home Alt' );
	...
	if(function_exists('wmvc_add_wp_image')) {
    	...
    	set_theme_mod( 'change_footer_logo', get_stylesheet_directory_uri() .'/assets/images/logo5.png');
				
		$custom_logo_id = wmvc_add_wp_image(get_stylesheet_directory() .'/assets/images/fav.jpg');
		update_option( 'site_icon', $custom_logo_id );
	}

	set_theme_mod( 'footer_content', esc_html__('Aenean sollicitudin, lorem quis bibend auctor, nisi elit consequat ipsum, necittis sem nibh id elit. Duis sed odio enim.', 'real-estate-golden') );
	...
	if(true){
		...
		set_theme_mod('sign_in_button_text', esc_html__('Sign In', 'real-estate-golden'));
