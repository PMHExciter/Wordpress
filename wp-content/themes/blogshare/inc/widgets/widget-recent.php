 ...
 * @since      1.0.0
 */
class BlogShare_Recent_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-blogshare-recent widget-posts-thumbnail',
			'description' => __( 'Display recent posts with thumbnails.', 'blogshare' )
		);

		// Create the widget.
		parent::__construct(
			'blogshare-recent', // $this->id_base
			__( '&raquo; Recent Posts', 'blogshare' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $recent = get_transient( 'blogshare_recent_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'blogshare_recent_widget_' . $this->id, $recent );
	...
	function flush_widget_transient() {
		delete_transient( 'blogshare_recent_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Recent Posts', 'blogshare' ),
			'limit'     => 6,
			'show_number' => true
		);
